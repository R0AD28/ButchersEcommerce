...
public class IdempotencyService {
    ...
    public void reserve(String key, Long userId) {
        ...
        String redisKey = "idempotency:catalog:" + userId + ":" + key.trim();
    ...
    public void release(String key, Long userId) {
        if (key != null) redisTemplate.delete("idempotency:catalog:" + userId + ":" + key.trim());
    }
    public void complete(String key, Long userId) {
        redisTemplate.opsForValue().set("idempotency:catalog:" + userId + ":" + key.trim(), "COMPLETED",
                Duration.ofHours(properties.idempotency().ttlHours()));
    }
}
